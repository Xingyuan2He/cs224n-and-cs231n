...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        # embed iput to (length,batch_size,e_char)
        char_embeded_input = self.decoderCharEmb(input)
        # hiddent_states:(len, batch_size , hidden_size)
        hiddent_states, dec_hidden = self.charDecoder(
            char_embeded_input, dec_hidden)
        scores = self.char_output_projection(hiddent_states)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        score, _ = self.forward(
            char_sequence[:-1], dec_hidden)  # delete <END>,(length-1,b,vocab_size)

        CE_layer = nn.CrossEntropyLoss(
            ignore_index=self.padding_idx, reduction='sum')
        # input(b,v_size,l)  target(b,l)
        loss = CE_layer(score.permute(1, 2, 0),
                        char_sequence[1:].permute(1, 0))
        return loss
        # END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        cur_char = torch.tensor(
            [[start_idx] * batch_size], device=device)  # 1,b tensor

        for i in range(max_length):
            scores, states = self.forward(cur_char, states)
            cur_char = scores.argmax(-1)
            output_words += [cur_char]

        output_words = torch.cat(output_words).t().tolist()
        # output_words:(b,l)
        for char_idx_seq in output_words:
            word_str = ""
            for char_idx in char_idx_seq:
                if char_idx == end_idx:
                    break
                word_str += self.target_vocab.id2char[char_idx]
            decodedWords.append(word_str)

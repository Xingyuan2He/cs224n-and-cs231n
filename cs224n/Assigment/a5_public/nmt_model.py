...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.model_embeddings_source = ModelEmbeddings(
            word_embed_size, vocab.src)
        self.model_embeddings_target = ModelEmbeddings(
            word_embed_size, vocab.tgt)
        ...
        self.decoder = nn.LSTMCell(
            word_embed_size + self.hidden_size, hidden_size=self.hidden_size)
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # target_padded: (max_sentence_length, batch_size)
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(
            source, device=self.device)  # with '{' and '}'
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(
            target, device=self.device)  # with '{' and '}'
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(
            target_padded)  # (seq_l,b) ->(seq_l,b,e)
        # 3
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        (dec_hidden, dec_cell) = dec_state[0], dec_state[1]  # d_h:(b,h)
        ...
        # COPY OVER YOUR CODE FROM ASSIGNMENT 4
        alpha_t = nn.functional.softmax(e_t, dim=1)  # (b,seq_len)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[
        ...
        src_sents_var = self.vocab.src.to_input_tensor_char(
            [src_sent], self.device)
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            y_tm1 = self.vocab.tgt.to_input_tensor_char(
                list([hyp[-1]] for hyp in hypotheses), device=self.device)
    ...
    def load(model_path: str, no_char_decoder=False):
        ...
        model = NMT(vocab=params['vocab'],
                    no_char_decoder=no_char_decoder, **args)

...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        X = self.words2charindices(sents)
        # denotes x in pdf
        X_padded = pad_sents_char(X, self.char_pad)
        # x_padded: shape of (batch_size, max_sentence_length, max_word_length)

        return torch.tensor(X_padded, dtype=torch.long, device=device).transpose(0, 1).contiguous()

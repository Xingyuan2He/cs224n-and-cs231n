...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # 1:compute the shape of params:
        # to keep the result unchanged after conv
        pad_size = int((filter_size - 1) / 2)
        stride = 1
        # shape after conv:
        h_after_conv1 = int(
            1 + (input_dim[1] + 2 * pad_size - filter_size) / stride)
        w_after_conv1 = int(
            1 + (input_dim[2] + 2 * pad_size - filter_size) / stride)
        # shape after max-pooling 2*2
        h_after_pool = 1 + (h_after_conv1 - 2) / 2
        w_after_pool = 1 + (w_after_conv1 - 2) / 2
        ...
        self.params['W2'] = weight_scale * \
            np.random.randn(
                num_filters * int(h_after_pool * w_after_pool), hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        loss, dscores = softmax_loss(scores, y) + 0.5 * self.reg * \
            (np.sum(W1 ** 2) + np.sum(W2 ** 2) + np.sum(W3 ** 2))

        dx, grads['W3'], grads['b3'] = affine_backward(dscores, cache_3)

...
def batchnorm_backward(dout, cache):
    ...
    # cite ：https://www.jianshu.com/p/04bc8026d3c4
    dx_hat = dout * gamma
    dsigma = -0.5 * np.sum(dx_hat * (x - mean), axis=0) * \
...
def layernorm_backward(dout, cache):
    ...
    # cite ：https://www.jianshu.com/p/04bc8026d3c4
    dx_hat = dout * gamma
    dsigma = -0.5 * np.sum(dx_hat * (x - mean), axis=1).reshape(-1, 1) * \
        np.power(var + eps, -1.5)
    dmu = -np.sum(dx_hat / np.sqrt(var + eps), axis=1).reshape(-1, 1)
    dx = dx_hat / np.sqrt(var + eps) + 2.0 * dsigma * (x - mean) / N + dmu / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(h_out):
        for j in range(w_out):
            x_cur_conv_area = x_pad[:, :, i * stride:i *
                                    stride + hh, j * stride:j * stride + ww]
            for k in range(f):
                out[:, k, i, j] = np.sum(
                    x_cur_conv_area * w[k], axis=(1, 2, 3))
    ...
    for i in range(h_out):
        for j in range(w_out):
            ...
            for sample in range(n):
                dx[sample,:,i*stride:i*stride+hh,j*stride:j*stride+ww] += np.sum(dout[sample,:,i,j][:,None,None,None]*w,axis = 0)
            for cur_filter in range(f):
                dw[cur_filter,:,:,:] += np.sum(dout[:,cur_filter,i,j][:,None,None,None]*x_cur_conv_area,axis = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(h_out):
        for j in range(w_out):
            out[:,:,i,j] += np.max(x[:,:,i*stride:i*stride+ph,j*stride:j*stride+pw],axis = (2,3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(h_out):
        for j in range(w_out):
            max_element = out[:,:,i,j]
            x_cur_area = x[:,:,i*stride:i*stride+ph, j *stride:j*stride+pw]
            mask = x_cur_area==max_element[:,:,None,None]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    #the BN method used for images, we make mean and var through
    #N,H,W, and normalize channels
    n, c, h, w = dout.shape
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    x_hat = (x - mean) / np.sqrt(eps + var)
    x_hat = x_hat.reshape(n, c, h, w)
    cache = mean, var, x, G, gamma, beta, x_hat.copy(), eps
...
def spatial_groupnorm_backward(dout, cache):
    ...
    n, c, h, w = dout.shape
    mean, var, x, G, gamma, beta, x_halt, eps = cache
    
    dbeta = dout.sum((0, 2, 3),keepdims = True)
    dgamma = (dout * x_halt).sum((0, 2, 3),keepdims = True)
